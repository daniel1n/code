<% 
const senlontitle="四柱八字在线排盘"
%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml"><head>
<meta http-equiv="Content-Type" content="text/html; charset=gb2312">
<TITLE>四柱八字在线排盘_实用查询工具大全 - Powered by Senlon!</TITLE>

</head>
<body><!--#include file="../senlon/conn.asp"--><!--#include file="../senlon/getuc.asp"-->
...
<!--#include file=../senlon/yzy.asp-->
<!--#include file=../senlon/sizhu.asp-->
<!--#include file=../senlon/yy.asp-->
<!--#include file=../senlon/pp_bz.asp-->
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" class="b1" id="paipanTable">
    <tbody>
      ...
      <tr>
        ...
        <td><%=name%>　<strong>出生地：</strong><%=area%>　　<strong>排盘方式：</strong>普通方式排盘</td>
      </tr> <tr>
        <td>
          <strong>公历：</strong></td>
        <td><%=borntime%>　<%if taiyang="0" then%><%else%><br><strong>真太阳出生时间：</strong><%=truedate%>&nbsp;<%end if%><br><strong>农历：</strong><%call form_load(truedate,ntime,cyue,cri,cnian)
response.Write(ntime)%></td>
      </tr>
      <tr>
        <td colspan="2">&nbsp;</td>
      </tr>   <tr>
        <td><strong>胎元：</strong> </td>
        <td><% call taiy(tm,dm)%></td>
      </tr>   <tr>
        <td><strong>命宫：</strong></td>
        <td><%call mingg(dm,tdz,minggongx)%></td>
      </tr>   <tr>
        <td colspan="2">&nbsp;</td>
      </tr>   <tr>
        <td><%call xunkong(ygz,yxk,yxs)
	call xunkong(mgz,mxk,mxs)
	call xunkong(dgz,dxk,dxs)
	call xunkong(tgz,txk,txs)%><strong>旬空：</strong></td>
        <td><font color=blue><%=yxk%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=mxk%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=dxk%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=txk%></font></td>
      </tr>
      <tr>
        <td><strong>起运：</strong></td>
        <td>命主于出生后<%=qysj%>开始起运</td>
      </tr>
      <tr>
        <td><strong>交运：</strong>	</td>
        <td>命主于公历<%=zysj%>交运</td>
      </tr>
      <tr>
        <td valign="middle"><strong><%if sex=1 then
	response.write "乾造："
	else
	response.write "坤造："
	end if%> </strong> </td>
        <td><font color=blue><% call sshen(ty,dtg)%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <% call sshen(tm,dtg)%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
      <%response.write "日元"%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<%next%></td>
      ...
      <tr>
        <td><strong>纳音：</strong></td>
        <td><font color=#408000>
          <% call nayin(ygz)%>
&nbsp;&nbsp;
      <% call nayin(mgz)%>
&nbsp;&nbsp;
...
&nbsp;&nbsp;
      ...
      <tr>
        <td colspan="2"><strong>神煞：</strong></td>
      </tr>  <tr>
        <td colspan="2"><%
...
%></td>
      ...
      <tr>
        <td><strong>始于：</strong> </td>
        <td><%dayu=year(truedate)
...
分 （<a href="/jingdu/" target="_blank"><font color="red">地区经度查询</font></a>） </td>
</tr>
<tr id="fs2"  style="display:block">
<td>普通方式排盘无须输入地区经度值</td>
</tr>
<tr>
<td >
...
<tr>
<td ><input id=jp name="quanpai" type="radio" value="0" />
...
<td align="center"><input type="submit" value="在线八字排盘" name="submit" style="cursor:hand;" /> </td>
</tr>
</form>
</table><%end if%>
  <table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" class="b1" >
    <tbody>
      ...
      <tr>
        <td>
<P>四柱八字（又称“生辰八字”）以人出生的年、月、日、时为四柱，合四柱之干支为八字。具体是由“年干、年支”为年柱，“月干、月支”为月柱，“日干、日支”为日柱，“时干、时支”为时柱。共四种组合（四柱），八个干支所组成（八字），故称“四柱”或“八字”全称“四柱八字”。</P>
<P><FONT color=#0000ff><STRONG>比劫帮身：<BR></STRONG></FONT>比肩劫财与日干属同类之物，均可助日干之力，如甲见甲（比）、乙（劫）、寅（禄）、卯（刃）之类，正如一个人打不过人家，有兄弟帮忙或壮胆，就能打过人家。日干弱，不能胜任财官伤食之消耗，柱见比劫则为喜，如柱中财多身弱，喜比劫帮身克财；柱中官杀旺而身弱，喜比劫帮身敌官杀；柱中伤食旺而身弱，喜比劫帮身泄气也，行运亦如之。人命逢之，兄弟有情，朋友得助，社会关系良好，发达亦是必靠兄弟朋友之助。日干本强，又见比劫来帮身，则为祸也。</P>
...
<P><STRONG><FONT color=#0000ff>财多身弱：</FONT></STRONG><BR>又叫“财旺身衰”。柱中日干弱而偏正财之力强，日主不能胜任之，其财反不能享，如三岁小儿要挑一百斤东西。凡财多身弱者，而柱中又有官杀，则财生官杀来克日主，其祸不可胜言。或柱中又有食伤，泄尽日干元气生在财上，其祸亦重。财多身弱，宜见柱有比劫帮身为福，行运遇比劫则发，或柱有印星亦吉，唯须比劫制财以护印。 </P>
<P><STRONG><FONT color=#0000ff>贪财坏印：</FONT></STRONG><BR>又叫“财星破印”。日干弱，喜印星生扶日干，则不喜财星，因财可克印也。若柱中以印为用神，而逢柱中有财星冲、克印星，则为不吉之兆，人命逢此，一者背井离乡，二者职业不定，三者学业难就，四者因财致祸，五者早克母亲，六者体弱病多，七者经常搬迁，八者为人虚浮了无实学，九者婆媳不睦，以上诸等，必犯一二，又看此财印居于何柱而详言之。行运遇之，多主有灾，或丢掉公职（也许是下海），或因财丧命。凡财星破印，须有比劫制财方佳，行运亦同。</P>
